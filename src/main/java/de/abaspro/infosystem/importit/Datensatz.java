...
public class Datensatz {
	 ...
	 * 
	 * In den Kopffeldern nach dem ersten Schl�ssel gesucht, falls ein 2. angeben wird, 
	 * wird eine ImportitExeption geworfen.
	 * Als R�ckgabewert wird die Spaltenummer zur�ckgegeben 
	 * 
	 */
	private Integer checkKeyField(List<Feld> kopfFelder2) throws ImportitException {
		...
		for (Feld feld : kopfFelder2) {
			if (!feld.getKey().isEmpty()) {
				if (keyfield != 0) {
					throw new ImportitException("Es wurden mehrere Schl�ssel eingetragen! Es ist nur einer erlaubt");
				}
